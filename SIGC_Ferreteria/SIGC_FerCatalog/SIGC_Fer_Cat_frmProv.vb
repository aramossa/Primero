...
Public Class SIGC_Fer_Cat_frmProv
    ...
    Private Sub SIGC_Fer_Cat_frmProv_Load(sender As Object, e As EventArgs) Handles Me.Load
        mbCargar = False
    ...
    Private Sub LlenarGrilla()
        Try
            ...
            dt = BL.fListarProveedores(GS_EMP_ID)
            BL = Nothing
        ...
        If Not Me.grdDat.ActiveRow Is Nothing Then
            If Not Me.grdDat.ActiveRow.IsGroupByRow Then
                If Not Me.grdDat.ActiveRow.IsFilterRow Then
                    Me.txtId.Text = Me.grdDat.ActiveRow.Cells("PROV_ID").Value.ToString
                    Me.txtDoc.Text = Me.grdDat.ActiveRow.Cells("PROV_NRODOC").Value.ToString
                    Me.txtNom.Text = Me.grdDat.ActiveRow.Cells("PROV_RAZSOC").Value.ToString
                    Me.txtDir.Text = Me.grdDat.ActiveRow.Cells("PROV_DIRECCION").Value.ToString
                    Me.txtTel.Text = Me.grdDat.ActiveRow.Cells("PROV_TELEFONO").Value.ToString
                    Me.txtEma.Text = Me.grdDat.ActiveRow.Cells("PROV_EMAIL").Value.ToString
                    If Me.grdDat.ActiveRow.Cells("PROV_EST").Value.ToString = "A" Then
    ...
    Private Sub Pinta_InfraGrilla()
        With Me.grdDat
            ...
            Next
            ConfiguraColGrilla(Me.grdDat, 0, "Id.Prov.", 50, True, False, False)
            ConfiguraColGrilla(Me.grdDat, 1, "Empr", 50, True, True, False)
            ConfiguraColGrilla(Me.grdDat, 2, "RUC", 100, True, False, False)
            ConfiguraColGrilla(Me.grdDat, 3, "Razón Social", 200, False, False, False)
            ConfiguraColGrilla(Me.grdDat, 4, "Dirección", 200, True, False, False)
            ConfiguraColGrilla(Me.grdDat, 5, "Teléfono", 100, False, False, False)
            ConfiguraColGrilla(Me.grdDat, 6, "Email", 200, False, False, False)
            ConfiguraColGrilla(Me.grdDat, 7, "Cod.Est.", 50, True, True, False)
            ConfiguraColGrilla(Me.grdDat, 8, "Estado", 50, False, False, False)
    ...
    Private Sub GrabarToolStripMenuItem_Click(sender As Object, e As EventArgs) Handles GrabarToolStripMenuItem.Click
        ...
        Try
            ...
            If Nuevo Then
                ...
                End If
                loRes = BL.fManTabProv(GS_OPC_ADD, lnId, GS_EMP_ID, txtDoc.Text, txtNom.Text, txtDir.Text, txtTel.Text, txtEma.Text, lsEst)
            End If

            ' Modificar
            If Editando Then
                lnId = grdDat.ActiveRow.Cells("PROV_ID").Value.ToString
                loRes = BL.fManTabProv(GS_OPC_MOD, lnId, GS_EMP_ID, txtDoc.Text, txtNom.Text, txtDir.Text, txtTel.Text, txtEma.Text, lsEst)
            End If
